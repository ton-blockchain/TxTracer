...
export function parseTolkErrors(errorText: string): ParsedError[] {
  const errors: ParsedError[] = []

  // filename:line:column: error: message
  const errorRegex = /(\w+\.tolk):(\d+):(\d+):\s*(error|warning|info):\s*(.+)/g
...
export function convertTolkErrorsToMarkers(errors: ParsedError[]): monaco.editor.IMarkerData[] {
